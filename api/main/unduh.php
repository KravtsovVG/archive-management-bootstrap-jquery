...
<tbody>
	<?php
	if(!$download = mysql_query("SELECT * FROM last_download ORDER BY id_download DESC LIMIT 10")){
		die(mysql_error());
	};
	$i = 1;
	while($data = mysql_fetch_array($download)){
		if(!$query = mysql_query("SELECT * FROM document_detail dd NATURAL JOIN document d WHERE id_doc_detail = ". $data['id_doc_detail'])){
			die(mysql_error());
		};
